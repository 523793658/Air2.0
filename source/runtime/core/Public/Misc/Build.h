...
#if BUILD_DEBUG + UE_BUILD_DEVELOPMENT + UE_BUILD_TEST + UE_BUILD_SHIPPING != 1
	#error Exactly one of [BUILD_DEBUG UE_BUILD_DEVELOPMENT UE_BUILD_TEST UE_BUILD_SHIPPING] should be defined to be 1
#endif
...
#endif
#define USE_HANG_DETECTION (ALLOW_HANG_DETECTION && !WITH_EDITORONLY_DATA && !IS_PROGRAM && !BUILD_DEBUG && !ENABLE_PGO_PROFILE)
...
#define USE_HITCH_DETECTION (ALLOW_HITCH_DETECTION && !WITH_EDITORONLY_DATA && !IS_PROGRAM && !BUILD_DEBUG)
...
// All other configurations always create backups.
	...
	#define ENABLE_RHI_VALIDATION (BUILD_DEBUG || UE_BUILD_DEVELOPMENT)
#endif
