...
namespace Air
{
	...
	const SHAHash& getShaderFileHash(const TCHAR* filename)
	{
	...
	void ShaderCompilerEnvironment::merge(const ShaderCompilerEnvironment& other)
	{
		...
		{
			auto existing = mIncludeFileNameToContentsMap.find(it.first);
			if (existing != mIncludeFileNameToContentsMap.end())
			{
				TArray<ANSICHAR>& existingContents = existing->second;

				if (existingContents.size() > 0)
				{
					existingContents.removeAt(existingContents.size() - 1);
				}
	...
	void serializeConstantBufferInfo(ShaderSaveArchive& ar, const TMap<const TCHAR*, CachedConstantBufferDeclaration>& constantBufferEntries)
	{
		for (auto& it : constantBufferEntries)
		{
			for (TLinkedList<ConstantBufferStruct*>::TIterator structIt(ConstantBufferStruct::getStructList()); structIt; structIt.next())
			{
				...
				{
					const ConstantBufferStruct& Struct = **structIt;
					const TArray<ConstantBufferStruct::Member>& members = Struct.getMembers();
					int32 numMembers = members.size();
					ar.serialize(NULL, numMembers);
					for (int32 memberIndex = 0; memberIndex < members.size(); memberIndex++)
					{
						const ConstantBufferStruct::Member& member = members[memberIndex];
