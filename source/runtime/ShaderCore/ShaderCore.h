...
namespace Air
{
	...
	public:
		ShaderParameterMap() {}
		SHADER_CORE_API bool findParameterAllocation(const TCHAR* inParameterName, uint16& outBufferIndex, uint16& outBaseIndex, uint16& outSize) const;

		SHADER_CORE_API bool containsParameterAllocation(const TCHAR* inParameterName)const;
		SHADER_CORE_API void addParameterAllocation(const TCHAR* inParameterName, uint16 inBufferIndex, uint16 BaseIndex, uint16 size);

		SHADER_CORE_API void removeParameterAllocation(const TCHAR* parameterName);

		SHADER_CORE_API void verifyBindingAreComplete(const TCHAR* shaderTypeName, ShaderTarget target, class VertexFactoryType* inVertexFactoryType) const;
	...
	{
		TMap<wstring, TArray<ANSICHAR>> mIncludeFileNameToContentsMap;
		...
		{
			return ar << environment.mIncludeFileNameToContentsMap << environment.mDefinitions << environment.mCompilerFlags << environment.mRenderTargetOutputFormatsMap << environment.mResourceTableMap << environment.mResourceTableLayoutHashes;
		}

		SHADER_CORE_API void merge(const ShaderCompilerEnvironment& other);
	...
	struct ShaderCompilerOutput
	{
		...
		SHADER_CORE_API void generateOutputHash();
	...
	{
		wstring mDeclaration[SP_NumPlatforms];
	};

	extern SHADER_CORE_API void flushShaderFileCache();
	extern SHADER_CORE_API void verifyShaderSourceFiles();
	extern SHADER_CORE_API void getShaderIncludes(const TCHAR* filename, TArray<wstring>& includeFilenames, uint32 depthLimits = 7);
	extern SHADER_CORE_API const SHAHash& getShaderFileHash(const TCHAR* filename);
	extern SHADER_CORE_API const class SHAHash& getShaderFilesHash(const TArray<wstring>& filenames);
	...
	extern SHADER_CORE_API void serializeConstantBufferInfo(class ShaderSaveArchive& ar, const TMap<const TCHAR*, CachedConstantBufferDeclaration>& constantBufferEntries);

	extern SHADER_CORE_API bool loadShaderSourceFile(const TCHAR* filename, wstring& outFileContents);

	extern SHADER_CORE_API void loadShaderSourceFileChecked(const TCHAR* filename, wstring & outFileContents);

	extern SHADER_CORE_API wstring getRelativeShaderFilename(const wstring& inFilename);

	extern SHADER_CORE_API void initializeShaderType();
} 