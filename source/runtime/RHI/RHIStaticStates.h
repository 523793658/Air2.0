...
namespace Air
{
	...
	extern RHI_API CriticalSection StaticStateRHICriticalSection;
	...
	*/
	...
	public:
		...
		FInitStaticResourceRenderThreadTask(void(*InDoConstruct)(), ScopedEvent& InEvent)
	...
	*/
		...
		static RHIParamRefType getRHI()
		{
			...
			if (!StaticResource)
			{
				if (GIsRHIInitialized && GRHISupportsRHIThread)
				{
					...
					FStaticStateResource* ValueWas = (FStaticStateResource*)PlatformAtomics::interLockedCompareExchangePointer((void**)&StaticResource, NewStaticResource, nullptr);
					if (ValueWas)
					{
	...
	private:
		...
		class FStaticStateResource : public RenderResource
		{
		public:
			RHIRefType StateRHI;
			...
			{
				BOOST_ASSERT(!GIsRHIInitialized || !GRHISupportsRHIThread);
				StateRHI = InitializerType::createRHI();
			}
	...
	*/
	...
	{
	public:
		static DepthStencilStateRHIRef createRHI()
		{
	...
	*/
	...
	{
	public:
		static BlendStateRHIRef createRHI()
		{
			TStaticArray<BlendStateInitializerRHI::RenderTarget, 8> RenderTargetBlendStates;
			...
			return RHICreateBlendState(BlendStateInitializerRHI(RenderTargetBlendStates));
		}
	};
