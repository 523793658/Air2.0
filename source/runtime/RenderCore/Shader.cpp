...
namespace Air
{
	...
	const SHAHash& ShaderType::getSourceHash(EShaderPlatform shaderPlatform) const
	{
	...
	void ShaderType::initialize(const TMap<wstring, TArray<const TCHAR*>>& shaderFileToConstantBufferVariables)
	{
		if (!PlatformProperties::requiresCookedData())
		{
			...
			{
				ShaderType* type = *it;
				generateReferencedConstantBuffers(type->mSourceFilename, type->mName, shaderFileToConstantBufferVariables, type->mReferencedShaderParametersMetadatasCache);
				{
		...
		mCanary(ShaderMagic_Initialized)
	{
		...
		BOOST_ASSERT(mType);
		mSourceHash = mType->getSourceHash(mTarget.getPlatform());
		if (mVFType)
		{
			mVFSourceHash = mVFType->getSourceHash(mTarget.getPlatform());
		}
		for (TLinkedList<ShaderParametersMetadata*>::TIterator structIt(ShaderParametersMetadata::getStructList()); structIt; structIt.next())
		{
			if (initializer.mParameterMap.containsParameterAllocation(structIt->getShaderVariableName()))
			{
				...
				mConstantBufferParameters.push_back(new ShaderConstantBufferParameter());
	...
	bool Shader::serializeBase(Archive& ar, bool bShadersInline)
	{
		...
		if (ar.isLoading())
		{
			...
			mConstantBufferParameterStructs.empty(numConstantParamters);
			mConstantBufferParameters.empty(numConstantParamters);

			for (int32 structIndex = 0; structIndex < numConstantParamters; structIndex++)
			{
				ShaderParametersMetadata* Struct = nullptr;
				wstring structName;
				ar << structName;
				Struct = findConstantBufferStructByName(structName);
				BOOST_ASSERT(Struct);
				ShaderConstantBufferParameter* parameter = new ShaderConstantBufferParameter();
				ar << *parameter;

				mConstantBufferParameterStructs.add(Struct);
				mConstantBufferParameters.add(parameter);
			}
		}
		else
		{
			int32 numConstantParameters = mConstantBufferParameters.size();
			ar << numConstantParameters;
		...
		if (bShadersInline)
		{
			if (ar.isSaving())
			{
				BOOST_ASSERT(mResource->mTarget == mTarget);
				mResource->serialize(ar);
			}

			if (ar.isLoading())
			{
				mSerializedResource = new ShaderResource();
				mSerializedResource->serialize(ar);
				BOOST_ASSERT(mOutputHash == mSerializedResource->mOutputHash);
			}
		}
		else
		{
			if (ar.isLoading())
			{
				ShaderResourceId resourceId(mTarget, mOutputHash, mType->limitShaderResourceToThisType() ? mType->getName() : nullptr, mType->limitShaderResourceToThisType() ? mPermutationId : 0);

				ShaderResource* existingResource = ShaderResource::findShaderResourceById(resourceId);
	...
	void ShaderType::flushShaderFileCache(const TMap<wstring, TArray<const TCHAR*>>& shaderFileToConstantBufferVariables)
	{
		...
		generateReferencedConstantBuffers(mSourceFilename, mName, shaderFileToConstantBufferVariables, mReferencedShaderParametersMetadatasCache);
	...
	ShaderId::ShaderId(const SHAHash& inMaterialShaderMapHash, const ShaderPipelineType * inShaderPipeline, VertexFactoryType* inVertexFactoryType, ShaderType* inShaderType, int32 inPermutationId, ShaderTarget inTarget)
		:mMaterialShaderMapHash(inMaterialShaderMapHash)
		,mShaderPipeline(inShaderPipeline)
		,mShaderType(inShaderType)
		,mSourceHash(inShaderType->getSourceHash(inTarget.getPlatform()))
		...
		,mTarget(inTarget)
	{
		if (inVertexFactoryType)
		{
			...
			mVFSourceHash = inVertexFactoryType->getSourceHash(inTarget.getPlatform());
		}
		else
		{
	...
	bool SelfContainedShaderId::isValid()
	{
		...
		if (it != map.end() && mSourceHash == it->second->getSourceHash(mTarget.getPlatform()) && mSerializationHistory == it->second->getSerializationHistory())
		{
			VertexFactoryType* VFTypePtr = VertexFactoryType::getVFByName(mVertexFactoryTypeName);
			if (mVertexFactoryTypeName == TEXT("") || (VFTypePtr && mVFSourceHash == VFTypePtr->getSourceHash(mTarget.getPlatform()) && mVFSerializationHistory == *VFTypePtr->getSerializationHistory((EShaderFrequency)mTarget.mFrequency)))
			{
				return true;
			}
		}
	...
	ShaderResource* ShaderResource::findOrCreateShaderResource(const ShaderCompilerOutput& output, class ShaderType* specificType, int32 specificPermutationId)
	{
		const ShaderResourceId resourceId(output.mTarget, output.mOutputHash, specificType ? specificType->getName() : nullptr, specificPermutationId);
	...
	ShaderType::ShaderType(EShaderTypeForDynamicCast inShaderTypeForDynamicCast, const TCHAR* inName, const TCHAR* inSourceFilename, const TCHAR* inFunctionName, uint32 inFrequency, int32 inTotalPermutationCount, ConstructSerializedType inConstructSerializedRef, GetStreamOutElementsType inGetStreamOutElementsRef)
		...
		,mTotalPermutationCount(inTotalPermutationCount)
	...
	/*Archive& operator<<(Archive& ar, ShaderType*& ref)
	{
		if (ar.isSaving())
		{
			wstring shaderTypeName = ref ? wstring(ref->mName) : TEXT("");
			ar << shaderTypeName;
		}
		else if (ar.isLoading())
		{
			wstring shaderTypeName = TEXT("");
			ar << shaderTypeName;

			ref = nullptr;
			if (shaderTypeName != TEXT(""))
			{
				auto map = ShaderType::getNameToTypeMap();
				auto it = map.find(shaderTypeName);
				if (it != map.end())
				{
					ref = it->second;
				}
				...
				{
					AIR_LOG(LogShaders, Verbose, TEXT("ShaderType '%s' dependency was not found"), shaderTypeName.c_str())
				}
			}
		}
		return ar;
	...
	static SHAHash mShaderResourceDefaultHash;

	SHAHash& ShaderResource::filterShaderSourceHashForSerialization(const Archive& ar, SHAHash& hashToSerialize)
	{
		return mShaderResourceDefaultHash;
	}

	void dispatchComputeShader(RHIAsyncComputeCommandListImmediate& RHICmdList, Shader* shader, uint32 threadGroupCountX, uint32 threadGroupCountY, uint32 threadGroupCountZ)
	{
