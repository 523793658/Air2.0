...
const static char *RS_BORDER_EXTENSION_VS = "default.vs";
const static char *RS_BORDER_EXTENSION_FS = "border_extension.fs";

typedef struct _BorderExtensionRenderer_
{
	char* input_name;
	char* output_name;
	PiBool is_deploy;
	...
	char* U_SrcTex;
	char* U_Size;
}BorderExtensionRenderer;


static PiBool _init(PiRenderer *renderer, PiHash *resources)
{
	BorderExtensionRenderer* impl = renderer->impl;
	PiColor color = { 1.0f, 1.0f, 0.0f, 0.0f };
	if (!impl->is_deploy)
	{
		return FALSE;
	}
	...
	impl->material = pi_material_new(RS_BORDER_EXTENSION_VS, RS_BORDER_EXTENSION_FS);
...
{
	BorderExtensionRenderer* renderer = pi_new0(BorderExtensionRenderer, 1);

	return pi_renderer_create(ERT_BORDER_EXTENSION, "border_extension", _init, _resize, _update, _draw, renderer);
}

void PI_API pi_border_extension_renderer_deploy(PiRenderer* renderer, char* input_name, char* output_name)
{
	BorderExtensionRenderer* impl = (BorderExtensionRenderer*)renderer->impl;
