...
ParseUVAnimTask::ParseUVAnimTask()
: result_(nullptr)
{
...
void ParseUVAnimTask::SetParam(byte* data, uint size, const CallBackType& cb)
{
	data_ = data;
	size_ = size;
	cb_ = cb;
...
bool ParseUVAnimTask::Run()
{
	PiUVAnim* result = app_parse_load_uv_anim(data_, size_);
