...
ParseSkeletonTask::ParseSkeletonTask()
: result_(nullptr)
{
...
void ParseSkeletonTask::SetParam(byte* data, uint size, const CallBackType& cb)
{
	data_ = data;
	size_ = size;
	cb_ = cb;
...
bool ParseSkeletonTask::Run()
{
	PiSkeleton* result = app_parse_load_skeleton(data_, size_);
...
void ParseSkeletonTask::Callback()
{
